...
const Experience = () => {
  return (
    <div className='py-16 bg-gray-100 dark:bg-gray-950'>
      <SectionHeading title_1="Experience &" title_2="Education" description="My professional journey and academic background" />
      ...
      <div className="relative px-6 max-w-4xl mx-auto ">
        ...
        {experiences.map((item,index)=>(
            <div data-aos="zoom-in" data-aos-delay={index * 100} data-aos-anchor-placement="top-center" className={`relative flex flex-col md:flex-row gap-8 mb-12 ${index % 2===0? "md:flex-row-reverse":""}`} key={index}>
                {/* Timeline node */}
                    ...
                    <div className="bg-white dark:bg-gray-900 shadow-md rounded-xl p-6 hover:scale-105 transition-all duration-300">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {item.technologies.map((tech,index)=>(
                                <span className="text-xs px-2 py-1 rounded-md bg-indigo-500 text-white" key={tech}>{tech}</span>
                            ))}
                        </div>
                    </div>
                </div>
